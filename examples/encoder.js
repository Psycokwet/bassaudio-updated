...
var filename = path.join(__dirname, "Ocarina of Time- Gerudo Valley.mp3");
var LameModes = {
  Stereo: 2,
  Mono: 1,
};
...
setTimeout(() => {
  ...
  var URL = "sv6.vestaradio.com:8352,1";
  ...
  if (!isCast) {
    if (basslib.BASS_ErrorGetCode() === 2100) {
      // access denied - passwd incorrect
      return console.log(
        chalk.bgRed.white.bold("icecast ERROR reason:Access denied to icecast")
      );
    }

    console.log(
      `${chalk.bgRed.white.bold(
        "error at BASS_Encode_CastInit: "
      )} ${basslib.BASS_ErrorGetCode()}`
    );
  } else {
    console.log(
